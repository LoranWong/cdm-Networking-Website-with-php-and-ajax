				...
				.done(function(response, status, xhr) {
					if (response != "[]") {
						...
						$.each(json, function(index, val) {
							...
							details = $(val.details).text().length > DETAILS_LENGTH ? $(val.details).text().slice(0, DETAILS_LENGTH) : $(val.details).text();
							details += "...  ";
							...
							item.find('.item_hot_comment_scale').click(function(event) {
								...
								shortDetails = longDetails.length > DETAILS_LENGTH ? $(longDetails).text().slice(0, DETAILS_LENGTH) : $(longDetails).text();
